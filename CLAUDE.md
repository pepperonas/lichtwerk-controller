...
**Weißpunkt kalibriert (2026-08-02, vierter Durchgang).** Der Gelb-/Grünstich kam nicht aus dem Blend-Verfahren, sondern aus dem **Ziel**: `(255,255,255)` ist auf einem WS2812B **kein Weiß**. Die grüne Die ist pro PWM-Schritt rund doppelt so lichtstark wie die rote, Blau etwa halb — gleiche Duty rendert also grünstichig, und der Weg dorthin führt durch Gelb. Neu: **`WHITE_POINT = (255, 178, 217)`** (Grün runter, Blau leicht über Grün), konfigurierbar über `config.json` → `iris_wash.white_point`, weil der Punkt strip-spezifisch ist. Farbpfad jetzt durchgehend **Rot → Warmrosa → Weiß ohne Gelb-/Grün-Passage** (Test läuft den ganzen Blend ab und prüft jede Stufe gegen den Gelb-Grün-Keil). Strom fällt nebenbei von 16,8 auf **15,4 A**, weil weniger Grün/Blau getrieben wird.

**Weiß als Blend statt Addition (dritter Durchgang).** Der additive Ansatz erzeugte genau den Regenbogen, den er vermeiden sollte: **Rot klippt bei 255, Grün und Blau laufen weiter** → jede Schimmer-Bande und jeder Spark bekam einen breiten Hof Rot→Orange→Gelb→Weiß. Bei 3 Banden und 18 Sparks las das als Warm-Regenbogen statt als rote Warnung.
